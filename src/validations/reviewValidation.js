...
const createReviewValidations = async function (req, res, next) {
    try {
        ...
        if(!checkBookId)  {
            return res.status(400).send({ status: false, message: "write valid bookId that are present in your collections" });
        }
        let data = req.body;
        ...
        if (typeof data.rating != "number") {
          ...
          if ((data.rating<0) || (data.rating>5)) {
            return res.status(400).send({ status: false, message: "You can give rating more than 0  or less than five in number only" });
          }
